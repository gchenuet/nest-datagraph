...
$sql = "SELECT date, city_curr_hum, nest_curr_hum, nest_targ_hum from status WHERE (DATE(date) BETWEEN (date_sub(date('$date'), INTERVAL 1 DAY)) AND CURDATE());";
...
while($row =mysqli_fetch_assoc($result))
{
    ...
    $date_array = "Date(".date('Y', $phpdate).",".date('n', strtotime("-1 month", $phpdate)).",".date('d', $phpdate)
        .",".date('H', $phpdate).",".date('i', $phpdate).")";
    $sql_array["rows"][] = array("c" => array(array("v" => $date_array), array("v" => $row["nest_curr_hum"]),
