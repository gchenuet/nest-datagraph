...
$sql = "SELECT SUM(nest_heat_state) AS IS_HEATING, SUM(if(auto_away = '0', 1, 0)) AS AT_HOME, SUM(if(auto_away = '1', 1, 0)) AS AUTO_AWAY, 
        (SELECT COUNT(*) AS leaf FROM (SELECT leaf FROM status WHERE leaf = 1 AND (DATE(date) BETWEEN (date('".$_GET['start']."')) AND (date('".$_GET['end']."'))) 
        ) status) AS LEAF FROM status WHERE (DATE(date) BETWEEN (date('".$_GET['start']."')) AND (date('".$_GET['end']."')));";
