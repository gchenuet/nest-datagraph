...
$sql = "SELECT date, nest_targ_temp, nest_curr_temp, city_curr_temp, away_temperature_low from status WHERE (DATE(date) BETWEEN (date('".$_GET['start']."')) AND (date('".$_GET['end']."')));";
$result = mysqli_query($connection, $sql) or die("Error in Selecting " . mysqli_error($connection));
$sql_array = array("cols" => array(array("label"=>"Date", "type"=>"datetime"),array("label"=>"Target","type"=>"number"), 
                                   array("label"=>"Interior","type"=>"number"), array("label"=>"Exterior","type"=>"number"), array("label"=>"Away target","type"=>"number")));
while($row = mysqli_fetch_assoc($result))
{
    ...
    $sql_array["rows"][] = array("c" => array(array("v" => $date_array), array("v" => $row["nest_targ_temp"]), 
                                              array("v" => $row["nest_curr_temp"]),array("v" => $row["city_curr_temp"]),array("v" => $row["away_temperature_low"])));
}
