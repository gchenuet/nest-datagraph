...
$sql = "SELECT date, nest_targ_temp, nest_curr_temp, city_curr_temp from status WHERE (DATE(date) BETWEEN (date_sub(date('$date'), INTERVAL 1 DAY)) AND CURDATE());";
...
while($row =mysqli_fetch_assoc($result))
{
    ...
    $date_array = "Date(".date('Y', $phpdate).",".date('n', strtotime("first day of -1 month", $phpdate)).",".date('d', $phpdate).",".date('H', $phpdate).",".date('i', $phpdate).")";
